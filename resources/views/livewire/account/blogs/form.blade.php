    <div class="card mb-6">
      ...
      <div class="card-content">
        <div class="max-w-4xl mx-auto p-3 border rounded">
              ...
              @if($formable)
                @forelse ($formable as $k => $form)
                 ...
                 @elseif ($form['type']=='tag')
                   <x-tag label="{{form_labels($form['label'])}}" wire:model="{{$k}}" />

                 @elseif ($form['type']=='date')
                   <x-date label="{{form_labels($form['label'])}}" wire:model="{{$k}}" />

                 @elseif ($form['type']=='file')
                 <div>
                  <label for=""label="" >
                    <span> {{form_labels($form['label'])}}</span><br>
                     <input type="file" wire:model="{{$k}}" />
                   </label> <br>
                 ...
                 <div class="col-span-3">
                  <x-mary-editor label="{{form_labels($form['label'])}}" wire:model="{{$k}}" />
                  <x-input-error :messages="$errors->get($k)" />
                </div>
