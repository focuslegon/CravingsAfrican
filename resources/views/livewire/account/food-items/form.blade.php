            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 ">
              @if($formable)
                @forelse ($formable as $k => $form)
                 ...
                 <x-tag label="{{form_labels($form['label'])}}" wire:model="{{$k}}" hint="Enter the tags separated by commas" />
                 ...
                 @endphp
                  <div class="col-span-2">
                    <x-select.styled :options="$options" multiple label="{{form_labels($form['label'])}}" wire:model="{{$k}}" select="label:label|value:value"/>
                  </div>
